/**
 * BarberDashboard — rediseño pixel-perfect al template
 * Negro · Naranja · Cards premium · iPhone optimized
 */
...
import { Clock, X, Scissors, Phone, Mail, Calendar, Plus, ChevronRight, TrendingUp, Bell } from 'lucide-react'
...
const CSS = `
  ...
  @keyframes glow    { 0%,100%{box-shadow:0 0 0 0 rgba(255,107,26,0.35)} 50%{box-shadow:0 0 0 8px rgba(255,107,26,0)} }
...
function apptStart(a) {
  ...
  const [h,mn]  = a.startTime.split(':').map(Number)
  return new Date(y,m-1,d,h,mn,0)
}
function apptEnd(a) {
  const [y,m,d] = a.date.split('-').map(Number)
  const [h,mn]  = a.endTime.split(':').map(Number)
  return new Date(y,m-1,d,h,mn,0)
}
...
function Avatar({ name, photoURL, size=40, fontSize=14, highlight=false }) {
  ...
  return (
    <div style={{
      ...
      background:CARD2, border:`2px solid ${highlight?ORANGE:BORDER}`,
      display:'flex', alignItems:'center', justifyContent:'center',
      fontWeight:800, fontSize, color:highlight?ORANGE:TXT2,
      boxShadow:highlight?`0 0 0 3px ${ORANGE}30`:'none',
    }}>
      {photoURL
  ...
  useEffect(() => {
    function calc() {
      ...
      if (now>=start&&now<=end) {
        ...
        setLabel(`${m}:${String(sec).padStart(2,'0')} restantes`)
        return
      }
      if (now<start) {
        const s=differenceInSeconds(start,now), m=Math.floor(s/60)
        setLabel(m>=60?`en ${Math.floor(m/60)}h ${m%60}m`:`en ${m}m`)
      }
    }
    calc()
...
function ClientModal({ appt, allAppts, onClose, onReschedule, onCancel }) {
  ...
  const related = allAppts
    .filter(a=>(appt.clientId&&a.clientId===appt.clientId)||(!appt.clientId&&a.clientEmail&&a.clientEmail===appt.clientEmail))
  ...
  const svcMap  = {}
  related.forEach(a=>a.services?.forEach(s=>{svcMap[s.name]=(svcMap[s.name]||0)+1}))
        ...
        {isNow && (
          <div style={{background:`linear-gradient(135deg,${ORANGE},#FF8C42)`,padding:'13px 20px',borderRadius:'22px 22px 0 0',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              ...
              <span style={{color:'#fff',fontWeight:800,fontSize:11,letterSpacing:'0.12em'}}>ATENDIENDO AHORA</span>
            </div>
        ...
        <div style={{padding:'20px 20px 40px'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
            <div style={{display:'flex',alignItems:'center',gap:14}}>
              ...
              <div>
                <p style={{color:TXT,fontWeight:800,fontSize:18,margin:'0 0 5px'}}>{appt.clientName}</p>
                {appt.isGuest && (
                  <span style={{background:CARD2,color:TXT2,fontSize:10,padding:'2px 8px',borderRadius:10,fontWeight:700,border:`1px solid ${BORDER}`}}>Invitado</span>
                )}
              </div>
            </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
            {appt.clientEmail && (
              <div style={{display:'flex',alignItems:'center',gap:10,background:CARD2,borderRadius:12,padding:'10px 14px'}}>
                <Mail size={14} color={TXT3}/><span style={{color:TXT2,fontSize:13}}>{appt.clientEmail}</span>
              </div>
            )}
            {phone && (
              <div style={{display:'flex',alignItems:'center',gap:10,background:CARD2,borderRadius:12,padding:'10px 14px'}}>
          ...
          <div style={{background:BG,border:`1.5px solid ${isNow?`${ORANGE}44`:BORDER}`,borderRadius:16,padding:14,marginBottom:14}}>
            <p style={{color:TXT3,fontSize:10,fontWeight:700,letterSpacing:'0.1em',marginBottom:12}}>CITA</p>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:14}}>
            {[
              {l:'Visitas',  v:visits},
              {l:'Total $',  v:formatCurrency(spent)},
              {l:'Favorito', v:topSvc?topSvc[0]:'—',sm:true},
            ].map(s=>(
          ...
          {related.length>1 && (
            <div style={{marginBottom:14}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
                <p style={{color:TXT3,fontSize:10,fontWeight:700,letterSpacing:'0.1em',margin:0}}>HISTORIAL</p>
                {related.length>5 && (
              ...
              <div style={{display:'flex',flexDirection:'column',gap:6}}>
                {items.map(a=>{
                  const borderColor = a.bookingStatus==='confirmed'?GREEN:a.bookingStatus==='cancelled'?'#EF4444':BORDER
                  return (
                    <div key={a.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 12px',background:BG,border:`1px solid ${a.id===appt.id?`${ORANGE}44`:BORDER}`,borderLeft:`3px solid ${borderColor}`,borderRadius:12,opacity:a.id===appt.id?1:0.65}}>
                      <div>
                        <p style={{color:a.id===appt.id?TXT:TXT2,fontWeight:a.id===appt.id?700:500,fontSize:13,margin:'0 0 2px'}}>
                          {a.date?format(parseLocalDate(a.date),'MMM d, yyyy'):'—'}{a.id===appt.id?' · Hoy':''}
                        </p>
                        <p style={{color:TXT3,fontSize:11,margin:0}}>{a.services?.map(s=>s.name).join(', ')}</p>
                      </div>
                      <div style={{textAlign:'right'}}>
                        <p style={{color:a.paymentStatus==='paid'?GREEN:TXT2,fontWeight:700,fontSize:13,margin:'0 0 3px'}}>{formatCurrency(a.totalWithTip||a.totalPrice)}</p>
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:70,background:'rgba(0,0,0,0.9)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={onClose}>
      <div style={{width:'100%',maxWidth:360,background:CARD,borderRadius:22,border:`1px solid ${BORDER}`,padding:24,...F}} onClick={e=>e.stopPropagation()}>
        <p style={{color:TXT,fontWeight:800,fontSize:18,marginBottom:6}}>¿Cancelar cita?</p>
        ...
        <div style={{display:'flex',gap:10}}>
          <button onClick={onClose} style={{flex:1,padding:'13px',borderRadius:14,background:'transparent',border:`1px solid ${BORDER}`,color:TXT2,fontWeight:600,cursor:'pointer',...F}}>Mantener</button>
  ...
  return (
    <button onClick={onClick}
      style={{
        ...
        background:isCurrent?`${ORANGE}10`:CARD2,
        border:`1px solid ${isCurrent?`${ORANGE}40`:BORDER}`,
        opacity:isDone?0.5:1, transition:'all 0.15s', marginBottom:8,
      }}>
...
export default function BarberDashboard() {
  ...
  const greeting = now.getHours()<12?'Buenos días,':now.getHours()<17?'Buenas tardes,':'Buenas noches,'

  function handleReschedule(appt) {
    setSelectedAppt(null)
    navigate('/barber/calendar',{state:{rescheduleId:appt.id}})
  }
    ...
    <BarberLayout>
      ...
      <div style={{background:BG,minHeight:'100vh',paddingBottom:80,...F}}>
        <div style={{padding:'16px 18px',maxWidth:640,margin:'0 auto'}}>
          ...
          <div className="fade-up" style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:22}}>
            <div style={{display:'flex',alignItems:'center',gap:14}}>
              ...
              <div>
                ...
                <p style={{color:TXT3,fontSize:10,margin:'2px 0 0',fontWeight:700,letterSpacing:'0.08em'}}>BARBERO</p>
              </div>
            </div>
            <button style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:12,padding:'9px',color:TXT2,cursor:'pointer',display:'flex',position:'relative'}}>
              <Bell size={18}/>
            </button>
          </div>
            ...
            {(todayEarned+todayProjected)>0 && (
              <div style={{marginTop:14}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
                  <span style={{color:TXT3,fontSize:11,fontWeight:600}}>Ganado</span>
                  <span style={{color:TXT2,fontSize:11,fontWeight:600}}>Proyectado {formatCurrency(todayProjected)}</span>
                </div>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                <div>
                  ...
                  <div style={{background:'rgba(0,0,0,0.2)',borderRadius:20,padding:'5px 12px',display:'inline-flex',alignItems:'center',gap:6}}>
                    ...
                    <span style={{color:'rgba(255,255,255,0.9)',fontWeight:700,fontSize:12,fontVariantNumeric:'tabular-nums'}}><Countdown appt={currentAppt}/></span>
                  </div>
                </div>
                <div style={{textAlign:'right'}}>
          ...
          <div className="fade-up" style={{marginBottom:12}}>
            ...
            {todayAppts.length===0 ? (
              <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:'28px',textAlign:'center'}}>
                ...
                <p style={{color:TXT2,fontWeight:600,fontSize:14,margin:'0 0 4px'}}>No hay citas hoy</p>
                <p style={{color:TXT3,fontSize:13,margin:0}}>Disfruta el día o agrega un walk-in</p>
              </div>
            ) : (
              todayAppts.map(a=>(
          ...
          {/* ── New Appointment CTA ── */}
          <button className="fade-up" onClick={()=>navigate('/barber/calendar')}
            style={{
      ...
      {selectedAppt && (
        <ClientModal
          appt={selectedAppt} allAppts={allAppts}
