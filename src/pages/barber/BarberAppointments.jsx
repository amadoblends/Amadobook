...
function NewApptModal({onClose,barber,activeServices,availability,appointments}){
  ...
  return(
    <div style={{position:'fixed',inset:0,zIndex:70,background:'rgba(0,0,0,0.9)',display:'flex',alignItems:'center',justifyContent:'center',padding:16,animation:'fadeIn 0.15s ease'}} onClick={onClose}>
      <div style={{width:'100%',maxWidth:400,background:CARD,borderRadius:20,border:`1px solid ${BORDER}`,maxHeight:'88dvh',overflowY:'auto',animation:'slideUp 0.2s ease',...F}} onClick={e=>e.stopPropagation()}>
        {/* Header */}
                ...
                return(
                  <button key={svc.id} onClick={()=>setSelSvc(svc)}
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
                      ...
                      <div style={{width:15,height:15,borderRadius:'50%',border:`2px solid ${sel?ORANGE:BORDER}`,background:sel?ORANGE:'transparent',display:'flex',alignItems:'center',justifyContent:'center'}}>
...
function ApptCard({appt,onClick,formatTime}){
  ...
  return(
    <button onClick={onClick} className="fu"
      style={{width:'100%',textAlign:'left',cursor:'pointer',...F,background:appt.isWalkIn?`${WALKIN}05`:CARD2,border:`1px solid ${appt.isWalkIn?`${WALKIN}20`:BORDER}`,borderLeft:`3px solid ${appt.isWalkIn?WALKIN:s.c}`,borderRadius:12,padding:'10px 12px',marginBottom:6,display:'flex',alignItems:'center',gap:10,transition:'all 0.12s'}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <p style={{color:TXT2,fontSize:11,margin:'0 0 3px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{appt.services?.map(s=>s.name).join(', ')}</p>
        <div style={{display:'flex',alignItems:'center',gap:6}}>
          <span style={{color:TXT3,fontSize:10}}>{isToday(parseD(appt.date))?'Today':isTomorrow(parseD(appt.date))?'Tomorrow':format(parseD(appt.date),'MMM d, yyyy')}</span>
        ...
        <div style={{padding:'12px 14px',maxWidth:540,margin:'0 auto'}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
            ...
            <div style={{display:'flex',gap:6}}>
              ...
              <button onClick={()=>setShowNewModal(true)}
                style={{background:ORANGE,border:'none',borderRadius:8,padding:'6px 12px',color:'#fff',cursor:'pointer',display:'flex',alignItems:'center',gap:5,fontWeight:700,fontSize:12,...F,boxShadow:`0 3px 10px ${ORANGE}35`}}>
          ...
          {filtered.length===0?(
            <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:'32px 16px',textAlign:'center'}}>
              ...
              <p style={{color:TXT2,fontWeight:600,fontSize:13,margin:'0 0 4px'}}>{search?'No results':tab==='upcoming'?'No upcoming appointments':tab==='past'?'No past appointments':'No cancelled'}</p>
              {tab==='upcoming'&&!search&&(
                <button onClick={()=>setShowNewModal(true)}
