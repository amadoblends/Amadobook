  ...
  return (
    <button onClick={() => onChange(!value)}
      style={{
        ...
        boxShadow: value ? `0 0 12px ${ORANGE}44` : 'none',
      }}>
...
function SettingsModal({ open, onClose, slotDuration, setSlotDuration, bufferTime, setBufferTime, advanceDays, setAdvanceDays, minNotice, setMinNotice, onSave, saving }) {
  if (!open) return null
  const fmtNotice = n => n===0?'None':n<60?`${n} min`:`${n/60}h`
      ...
      <div style={{ width:'100%', maxWidth:520, background:CARD, borderRadius:'24px 24px 0 0', border:`1px solid ${BORDER}`, padding:'0 0 40px', maxHeight:'80vh', overflowY:'auto', animation:'slideUp 0.25s ease' }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ padding:'20px' }}>
          {[
            ...
            <div key={s.label} style={{ marginBottom:24 }}>
              ...
              <div style={{ display:'flex', gap:8, marginBottom:12 }}>
                {s.presets.map(v=>(
                  <button key={v} onClick={()=>s.set(v)}
                    style={{ flex:1, padding:'11px 4px', borderRadius:14, border:`1.5px solid ${s.value===v?ORANGE:BORDER}`, background:s.value===v?ORANGE:'transparent', color:s.value===v?'#fff':TXT2, fontWeight:700, fontSize:12, cursor:'pointer', ...F, transition:'all 0.15s', boxShadow:s.value===v?`0 4px 12px ${ORANGE}33`:'none' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10, background:BG, border:`1px solid ${BORDER}`, borderRadius:12, padding:'10px 14px' }}>
                <input type="number" value={s.value} onChange={e=>s.set(Math.max(0,+e.target.value))} min="0"
                  style={{ flex:1, background:'transparent', border:'none', outline:'none', color:TXT, fontSize:16, fontWeight:700, ...F }}/>
          ...
          <button onClick={onSave} disabled={saving}
            style={{ width:'100%', background:ORANGE, border:'none', borderRadius:22, padding:'16px', color:'#fff', fontWeight:700, fontSize:16, cursor:'pointer', ...F, boxShadow:`0 4px 24px ${ORANGE}44`, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
...
export default function BarberAvailability() {
      ...
      try {
        ...
        if (!aSnap.empty) {
          ...
          if (data.schedule) {
            ...
            const s={}
            DAYS.forEach(d=>{ s[d]={ enabled:(data.workingDays||[1,2,3,4,5,6]).includes(d), startTime:data.startTime||'09:00', endTime:data.endTime||'18:00', breaks:data.breaks||[] } })
            setSchedule(s)
          }
        }
  ...
  function addBlockedDate() {
    ...
    if (blockedDates.includes(newBlockDate)) { toast.error('Already blocked'); return }
    setBlockedDates(p=>[...p,newBlockDate].sort())
        ...
        <div style={{ padding:'16px 18px', maxWidth:600, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
            ...
            <div style={{ display:'flex', gap:8 }}>
              <button onClick={()=>setSettingsOpen(true)}
                style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:12, padding:'9px 12px', color:TXT2, cursor:'pointer', display:'flex', alignItems:'center', gap:6, fontSize:13, fontWeight:600, ...F }}>
                <Settings2 size={15}/> Rules
              </button>
              <button onClick={handleSave} disabled={saving}
                style={{ background:ORANGE, border:'none', borderRadius:12, padding:'9px 16px', color:'#fff', cursor:'pointer', display:'flex', alignItems:'center', gap:6, fontSize:13, fontWeight:700, ...F, boxShadow:`0 4px 14px ${ORANGE}44` }}>
          ...
          <div style={{ display:'flex', background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:3, marginBottom:20 }}>
            {['Weekly','Exceptions'].map(t=>(
              <button key={t}
                style={{ flex:1, padding:'10px', borderRadius:11, border:'none', fontWeight:700, fontSize:13, background:t==='Weekly'?ORANGE:'transparent', color:t==='Weekly'?'#fff':TXT2, cursor:'pointer', ...F, transition:'all 0.15s' }}>
                {t}
              </button>
            ))}
          </div>
                      ...
                      {(day.breaks||[]).map((b,i)=>(
                        <div key={i} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
                          ...
                          <button onClick={()=>removeBreak(d,i)}
                            style={{ width:36, height:36, borderRadius:10, background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.2)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', flexShrink:0, color:'#EF4444' }}>
                            <X size={14}/>
                          </button>
                        </div>
                      ))}
          ...
          {/* Blocked dates */}
          <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:20, padding:'18px', marginBottom:12 }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
              <p style={{ color:TXT, fontWeight:700, fontSize:15, margin:0 }}>Blocked Dates</p>
              <button onClick={()=>setShowBlockInput(v=>!v)}
                style={{ background:`${ORANGE}18`, border:`1px solid ${ORANGE}33`, borderRadius:10, padding:'7px 12px', color:ORANGE, fontSize:13, fontWeight:700, cursor:'pointer', ...F, display:'flex', alignItems:'center', gap:5 }}>
                <Plus size={13}/> Block Day
              </button>
            </div>

            {showBlockInput && (
              <div style={{ background:BG, border:`1px solid ${BORDER}`, borderRadius:14, padding:'14px', marginBottom:14 }}>
                <input type="date" value={newBlockDate} min={format(new Date(),'yyyy-MM-dd')} onChange={e=>setNewBlockDate(e.target.value)}
                  style={{ width:'100%', background:'transparent', border:'none', borderBottom:`1.5px solid ${BORDER}`, outline:'none', color:TXT, fontSize:16, fontWeight:600, padding:'8px 0', marginBottom:12, ...F }}/>
                <div style={{ display:'flex', gap:8 }}>
                  <button onClick={()=>setShowBlockInput(false)} style={{ flex:1, padding:'11px', borderRadius:14, background:'transparent', border:`1px solid ${BORDER}`, color:TXT2, fontWeight:600, cursor:'pointer', ...F }}>Cancel</button>
            ...
            {blockedDates.length===0 ? (
              <p style={{ color:TXT3, fontSize:13 }}>No blocked dates</p>
            ) : (
              <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                {blockedDates.map(date=>(
                  <div key={date} style={{ display:'flex', alignItems:'center', gap:7, padding:'7px 12px', background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:22 }}>
                    <span style={{ color:'#EF4444', fontSize:13, fontWeight:700 }}>
                      {format(new Date(date+'T12:00'),'MMM d, yyyy')}
                    </span>
                    <button onClick={()=>setBlockedDates(p=>p.filter(dd=>dd!==date))} style={{ background:'none', border:'none', color:'#EF4444', cursor:'pointer', display:'flex', padding:0 }}>
                      <X size={13}/>
                    </button>
                  </div>
                ))}
