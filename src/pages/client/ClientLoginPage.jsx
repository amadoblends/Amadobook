...
const CSS = `
  ...
  * { box-sizing: border-box; }
  .field { width:100%; background:transparent; border:none; border-bottom:1.5px solid ${BORDER}; outline:none; color:${TXT}; padding:10px 0; font-size:16px; font-family:'DM Sans',system-ui,sans-serif; transition:border-color 0.2s; }
  ...
  return (
    <div style={{ minHeight:'100dvh', background:BG, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'24px 20px', ...F }}>
      ...
      <div style={{ width:'100%', maxWidth:400, animation:'fadeUp 0.28s ease both' }}>
        ...
        {/* Header */}
        <p style={{ color:TXT2, fontSize:12, fontWeight:600, margin:'0 0 6px', letterSpacing:'0.06em' }}>WELCOME BACK</p>
        <h1 style={{ color:TXT, fontWeight:800, fontSize:28, margin:'0 0 32px', letterSpacing:'-0.5px', lineHeight:1.1 }}>
          Sign in to<br/><span style={{ color:ORANGE }}>your account.</span>
        </h1>

        {/* Google */}
        <button onClick={handleGoogle} disabled={gLoading}
          style={{ width:'100%', background:CARD, border:`1px solid ${BORDER}`, borderRadius:14, padding:'14px', display:'flex', alignItems:'center', justifyContent:'center', gap:10, cursor:'pointer', marginBottom:20, ...F, transition:'border-color 0.15s' }}>
          {gLoading
        ...
        <form onSubmit={handleLogin}>
          ...
          <div style={{ textAlign:'right', marginBottom:28 }}>
            <Link to={`/b/${barberSlug}/forgot-password`}
              style={{ color:TXT2, fontSize:12, fontWeight:600, textDecoration:'none' }}>
              Forgot password?
            </Link>
          </div>
        ...
        <p style={{ textAlign:'center', color:TXT2, fontSize:14, margin:0 }}>
          ...
          <Link to={`/b/${barberSlug}/register`} style={{ color:ORANGE, fontWeight:700, textDecoration:'none' }}>
            Create one
          </Link>
        </p>
      </div>
