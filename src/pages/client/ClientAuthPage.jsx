...
const CSS = `
  ...
  .cl-input {
    ...
    padding:12px 0 10px; font-size:16px; outline:none;
  ...
  .cl-label { display:block; font-size:11px; color:#888888; letter-spacing:0.1em; font-weight:700; text-transform:uppercase; margin-bottom:4px; }
  .cl-field { margin-bottom:24px; }
  .btn-primary { width:100%; background:#FF6B1A; color:#fff; border:none; border-radius:22px; padding:18px; font-size:16px; font-weight:700; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px; font-family:'DM Sans',system-ui,sans-serif; transition:opacity 0.15s; animation: glow 3s infinite; }
  .btn-primary:disabled { opacity:0.5; cursor:not-allowed; animation:none; box-shadow:none; }
  .btn-outline { width:100%; background:transparent; color:#888888; border:1.5px solid #2A2A2A; border-radius:22px; padding:16px; font-size:15px; font-weight:700; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px; font-family:'DM Sans',system-ui,sans-serif; }
  .err { background:rgba(239,68,68,0.1); border:1px solid rgba(239,68,68,0.2); border-radius:14px; padding:12px 16px; color:#EF4444; font-size:14px; margin-bottom:20px; font-weight:500; }
...
const Spin = () => <div style={{ width:18,height:18,border:'2.5px solid rgba(255,255,255,0.4)',borderTopColor:'#fff',borderRadius:'50%',animation:'spin 0.75s linear infinite' }}/>

const ScissorsIcon = () => (
  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#FF6B1A" strokeWidth="1.5" strokeLinecap="round">
  ...
  return (
    <form onSubmit={submit} className="fade-up">
      ...
      <div style={{ textAlign:'right', marginBottom:28 }}>
        <button type="button" onClick={()=>navigate(`/b/${barberSlug}/forgot-password`)} style={{ background:'none',border:'none',color:'#888',fontSize:13,fontWeight:700,cursor:'pointer',...F }}>Forgot password?</button>
      </div>
      <button className="btn-primary" type="submit" disabled={loading} style={{ marginBottom: 20 }}>{loading&&<Spin/>} {loading?'Signing in…':'Sign In'}</button>
      <p style={{ textAlign: 'center', color: '#888', fontSize: 14, margin: 0 }}>
        Don't have an account? <button type="button" onClick={toggleSignup} style={{ background:'none',border:'none',color:'#FF6B1A',fontSize:14,fontWeight:700,cursor:'pointer',...F }}>Sign up</button>
      </p>
    </form>
  )
}
...
function SignupForm({ onSuccess, toggleLogin }) {
  ...
  return (
    <form onSubmit={submit} className="fade-up">
      ...
      <div className="cl-field" style={{ marginBottom:32 }}><label className="cl-label">Confirm</label><input className="cl-input" type="password" value={f.confirm} onChange={set('confirm')} placeholder="Repeat password" required autoComplete="new-password"/></div>
      <button className="btn-primary" type="submit" disabled={loading} style={{ marginBottom: 20 }}>{loading&&<Spin/>} {loading?'Creating…':'Create Account'}</button>
      <p style={{ textAlign: 'center', color: '#888', fontSize: 14, margin: 0 }}>
        Already have an account? <button type="button" onClick={toggleLogin} style={{ background:'none',border:'none',color:'#FF6B1A',fontSize:14,fontWeight:700,cursor:'pointer',...F }}>Sign in</button>
      </p>
    </form>
  )
}
...
function GuestForm({ onGuestSuccess }) {
  ...
  return (
    <form onSubmit={submit} className="fade-up">
      {err && <div className="err">{err}</div>}
      <p style={{ color:'#888',fontSize:14,margin:'0 0 24px', fontWeight:500 }}>Your info is used only for this booking.</p>
      <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:24 }}>
      ...
      <div className="cl-field" style={{ marginBottom:32 }}><label className="cl-label">Phone (optional)</label><input className="cl-input" type="tel" value={f.phone} onChange={set('phone')} placeholder="(315) 000-0000" autoComplete="tel"/></div>
...
export default function ClientAuthPage() {
  ...
  return (
    <>
      <style>{CSS}</style>
      <div style={{ minHeight:'100dvh', background:'#0D0D0D', display:'flex', flexDirection:'column', alignItems: 'center', ...F, overflowX:'hidden', padding: '40px 24px' }}>

        <div style={{ width: 64, height: 64, borderRadius: 20, background: 'rgba(255,107,26,0.1)', border: '1px solid rgba(255,107,26,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 32, marginTop: '5vh' }}>
          <ScissorsIcon />
        </div>

        <div style={{ width: '100%', maxWidth: 400 }}>
          <h1 style={{ color: '#F5F5F5', fontSize: 32, fontWeight: 900, margin: '0 0 8px', letterSpacing: '-1px' }}>
            {tab === 'guest' ? 'Continue as Guest.' : tab === 'signup' ? 'Create Account.' : 'Welcome back.'}
          </h1>
          <p style={{ color: '#888888', fontSize: 16, margin: '0 0 32px', fontWeight: 500 }}>
          ...
          {tab !== 'guest' && (
            <>
              ...
              <button className="btn-outline" onClick={handleGoogle} disabled={googleL} style={{ marginBottom: 24 }}>
                <GoogleIcon/> {googleL ? 'Signing in…' : 'Continue with Google'}
              </button>
              <div style={{ display:'flex',alignItems:'center',gap:16,marginBottom:24 }}>
                <div style={{ flex:1,height:1,background:'#2A2A2A' }}/><span style={{ color:'#555555',fontSize:12, fontWeight:700 }}>OR EMAIL</span><div style={{ flex:1,height:1,background:'#2A2A2A' }}/>
              </div>
            </>
          )}
          ...
          <div style={{ marginTop: 40, textAlign: 'center' }}>
            <button onClick={()=>navigate(`/b/${barberSlug}`)}
              style={{ background:'none',border:'none',color:'#555555',fontSize:14,fontWeight:700,cursor:'pointer',...F }}>
              ← Back to barber page
            </button>
          </div>
        </div>
