/**
 * ClientRegisterPage — Migrated to Design System
  ...
  return (
    <div style={{ minHeight:'100dvh', background:'var(--bg)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'24px 20px', ...F }}>
      ...
      <div style={{ width:'100%', maxWidth:400, animation:'fadeUp 0.28s ease both' }}>
        ...
        {/* Header */}
        <p style={{ color:'var(--text-sec)', fontSize:12, fontWeight:600, margin:'0 0 6px', letterSpacing:'0.06em' }}>CREATE ACCOUNT</p>
        <h1 style={{ color:'var(--text-pri)', fontWeight:800, fontSize:28, margin:'0 0 32px', letterSpacing:'-0.5px', lineHeight:1.1 }}>
          Your best look<br/><span style={{ color:'var(--accent)' }}>starts here.</span>
        </h1>

        {/* Google */}
        <button onClick={handleGoogle} disabled={gLoading}
          style={{ width:'100%', background:'var(--card)', border:'1px solid var(--border)', borderRadius:14, padding:'14px', display:'flex', alignItems:'center', justifyContent:'center', gap:10, cursor:'pointer', marginBottom:20, boxShadow:'var(--shadow-sm)', ...F }}>
          {gLoading
        ...
        <form onSubmit={handleRegister}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:20 }}>
            <div>
              <label style={{ color:'var(--text-ter)', fontSize:10, fontWeight:700, letterSpacing:'0.1em', display:'block', marginBottom:8 }}>FIRST NAME *</label>
              <input type="text" value={form.firstName} onChange={set('firstName')} placeholder="Alex" autoComplete="given-name" className="field"/>
            </div>
            <div>
              <label style={{ color:'var(--text-ter)', fontSize:10, fontWeight:700, letterSpacing:'0.1em', display:'block', marginBottom:8 }}>LAST NAME</label>
          ...
          <div style={{ marginBottom:20 }}>
            <label style={{ color:'var(--text-ter)', fontSize:10, fontWeight:700, letterSpacing:'0.1em', display:'block', marginBottom:8 }}>PHONE</label>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', gap:10, marginBottom:28, cursor:'pointer' }} onClick={() => setAgreed(p => !p)}>
            <div style={{ width:20, height:20, borderRadius:6, border:`1.5px solid ${agreed?'var(--accent)':'var(--border)'}`, background:agreed?'var(--accent)':'transparent', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, marginTop:1, transition:'all 0.15s' }}>
            ...
            <p style={{ color:'var(--text-sec)', fontSize:13, margin:0, lineHeight:1.5 }}>
              I accept the <span style={{ color:'var(--accent)', fontWeight:600 }}>Terms & Conditions</span> and <span style={{ color:'var(--accent)', fontWeight:600 }}>Privacy Policy</span>
            </p>
          </div>
