...
export function HistoryPage() {
  ...
  useEffect(()=>{
    ...
    const unsub=onSnapshot(q,snap=>{setAppts(snap.docs.map(d=>({id:d.id,...d.data()})));setLoading(false)})
    return unsub
  },[user])
    ...
    <div style={{background:BG,minHeight:'100vh',paddingBottom:100,...F}}>
      ...
      <div style={{padding:'16px 18px',maxWidth:500,margin:'0 auto'}}>
        ...
        {history.length===0?(
          ...
          return(
            <div key={a.id} className="fade-up" style={{background:CARD,border:`1px solid ${BORDER}`,borderLeft:`3px solid ${cancelled?`${RED}40`:completed?`${GREEN}40`:BORDER}`,borderRadius:14,padding:'14px',marginBottom:8}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                <div>
                  <p style={{color:TXT,fontWeight:700,fontSize:13,margin:'0 0 2px'}}>{a.date?format(parseLocalDate(a.date),'MMM d, yyyy'):'—'}</p>
                  <p style={{color:TXT2,fontSize:12,margin:0}}>{formatTime?formatTime(a.startTime):a.startTime} · {formatDuration(a.totalDuration||0)}</p>
                </div>
                <div style={{textAlign:'right'}}>
                  <p style={{color:cancelled?TXT2:TXT,fontWeight:800,fontSize:14,margin:'0 0 4px',textDecoration:cancelled?'line-through':'none'}}>{formatCurrency(a.totalWithTip||a.totalPrice)}</p>
                  <span style={{fontSize:9,fontWeight:800,padding:'2px 7px',borderRadius:20,background:cancelled?`${RED}14`:completed?`${GREEN}12`:`${BORDER}`,color:cancelled?RED:completed?GREEN:TXT2,letterSpacing:'0.05em'}}>{(a.bookingStatus||'').toUpperCase()}</span>
                </div>
              </div>
              {a.services?.length>0&&<p style={{color:TXT2,fontSize:11,margin:'4px 0 0',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{a.services.map(s=>s.name).join(', ')}</p>}
              {a.tip>0&&<p style={{color:GREEN,fontSize:11,margin:'4px 0 0'}}>+{formatCurrency(a.tip)} tip</p>}
            </div>
          )
        })}
      </div>
