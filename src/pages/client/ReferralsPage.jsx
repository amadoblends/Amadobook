...
export function ReferralsPage() {
  ...
  const referralLink  = `${window.location.origin}/b/${barberSlug}?ref=${refCode}`
  ...
  function share() {
    if(navigator.share) navigator.share({title:`Book with ${barberSlug}`,url:referralLink})
    else copy()
  }
      ...
      <div style={{padding:'16px 18px',maxWidth:500,margin:'0 auto'}}>
        ...
        <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:'16px 18px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div>
            ...
            <p style={{color:TXT,fontWeight:800,fontSize:18,margin:0,fontFamily:'monospace',letterSpacing:'0.1em'}}>{refCode.toUpperCase()}</p>
          </div>
