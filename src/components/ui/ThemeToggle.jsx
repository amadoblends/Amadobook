...
export default function ThemeToggle({ showAccents = true }) {
  const { theme, toggleTheme, accent, setAccent, role, barberAccents, timeFormat, setTimeFormat } = useTheme()
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:36, height:36, borderRadius:10, background:'var(--accent)20', display:'flex', alignItems:'center', justifyContent:'center' }}>
            {isLight ? <Sun size={18} color="var(--accent)"/> : <Moon size={18} color="var(--accent)"/>}
          </div>
          <div>
        ...
        <button onClick={toggleTheme}
          ...
          <div style={{ width:22, height:22, borderRadius:'50%', background:'var(--accent-inv)', boxShadow:'0 1px 4px rgba(0,0,0,0.3)' }}/>
        </button>
      </div>

      {/* Time format */}
      <div>
        <p style={{ color:'var(--text-sec)', fontSize:10, fontWeight:700, letterSpacing:'0.1em', marginBottom:8, fontFamily:'Monda,sans-serif' }}>TIME FORMAT</p>
        <div style={{ display:'flex', background:'var(--bg)', borderRadius:12, padding:3, border:'1px solid var(--border)' }}>
          {[['12h','12h (AM/PM)'],['24h','24h']].map(([val,lbl]) => (
            <button key={val} onClick={()=>setTimeFormat(val)}
              style={{ flex:1, padding:'9px', borderRadius:10, fontWeight:700, fontSize:13, background:timeFormat===val?'var(--accent)':'transparent', color:timeFormat===val?'var(--accent-inv)':'var(--text-sec)', border:'none', cursor:'pointer', fontFamily:'Monda,sans-serif' }}>
              {lbl}
            </button>
          ))}
        </div>
          ...
          <div style={{ display:'flex', gap:10 }}>
            {barberAccents.map(a => (
              <button key={a.id} onClick={()=>setAccent(a.color)}
                ...
                <div style={{ width:26, height:26, borderRadius:'50%', background:a.color, border:`3px solid ${accent===a.color?'var(--accent-inv)':'transparent'}`, boxShadow:accent===a.color?`0 0 0 2px ${a.color}`:'none' }}/>
